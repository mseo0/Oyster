...
function createWindow() {
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: true,          // renderer runs sandboxed; preload uses only ipcRenderer
      webviewTag: false,      // no <webview> embedding
      spellcheck: false,
    },
  });
  win.loadFile(path.join(__dirname, '..', 'renderer', 'index.html'));
  // Lock the renderer to its local content: deny popups and any navigation away
  // from the bundled UI, so a compromised renderer can't load remote pages.
